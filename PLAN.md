...
| 18 | Policy routing (all types) ✅ *client-IP v0.4.0, port/dest-IP/domain v0.6.0, transparent-domain v0.10.0* | client-IP / dest-IP / port / domain → WAN, first-match-wins, comma lists + port ranges; down-backend falls back. Domain matches SOCKS hostnames **and transparent traffic via TLS SNI (443) + HTTP Host (80)** — verified on hardware (ifconfig.me pinned by SNI). |
